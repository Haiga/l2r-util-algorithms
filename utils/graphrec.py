...
def inferenceSparse(phase, user_batch, item_batch, uflength, iflength, user_num, item_num, UReg=0.05, IReg=0.1):
    umfl1 = tf.get_variable('umfl1', [uflength, MFSIZE], dtype=tf.float64,
                            initializer=tf.random_normal_initializer(stddev=0.01))
    umfl1b = tf.get_variable('umfl1b', [1, MFSIZE], dtype=tf.float64, initializer=tf.constant_initializer(0))

    imfl1 = tf.get_variable('imfl1', [iflength, MFSIZE], dtype=tf.float64,
                            initializer=tf.random_normal_initializer(stddev=0.01))
...
def GraphRec(train, test, ItemData=False, UserData=False, Graph=False, Dataset='100k'):
    AdjacencyUsers = np.zeros((USER_NUM, ITEM_NUM),
                              dtype=np.float32)  # np.asarray([[0 for x in range(ITEM_NUM)] for y in range(USER_NUM)],dtype=np.float16)
    DegreeUsers = np.zeros((USER_NUM, 1),
                           dtype=np.float32)  # np.asarray([[0 for x in range(1)] for y in range(USER_NUM)],dtype=np.float16)

    AdjacencyItems = np.zeros((ITEM_NUM, USER_NUM),
                              dtype=np.float32)  # np.asarray([[0 for x in range(USER_NUM)] for y in range(ITEM_NUM)],dtype=np.float16)
    DegreeItems = np.zeros((ITEM_NUM, 1),
                           dtype=np.float32)  # np.asarray([[0 for x in range(1)] for y in range(ITEM_NUM)],dtype=np.float16)
    ...
    infer, regularizer = inferenceDense(phase, user_batch, item_batch, w_user, w_item, user_num=USER_NUM,
                                        item_num=ITEM_NUM)
...
def GraphRecSparse(train, test, ItemData=False, UserData=False, Graph=False, Dataset='aiv'):
    ...
    infer, regularizer = inferenceSparse(phase, user_batch, item_batch, uflength=UserFeaturesLength,
                                         iflength=ItemFeaturesLength, user_num=USER_NUM, item_num=ITEM_NUM)
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(EPOCH_MAX * samples_per_batch):
            ...
            usersdatain = UserFeatures[np.asarray(users, dtype=np.int32),
                          :].tocoo()  # np.take(UserFeatures, users,axis=0)
            itemsdatain = ItemFeatures[np.asarray(items, dtype=np.int32),
                          :].tocoo()  # np.take(ItemFeatures, items,axis=0)
...
def get_UserData100k():
    ...
    df = pd.read_csv('toy_data/ml100k/u.user', sep='|', header=None, names=col_names, engine='python')
    del df["PostCode"]
    df["user"] -= 1
    df = pd.get_dummies(df, columns=["age", "gender", "occupation"])
    del df["user"]
...
def get_ItemData100k():
    col_names = ["movieid", "movietitle", "releasedate", "videoreleasedate", "IMDbURL"
        ...
        , "War", "Western"]
    df = pd.read_csv('toy_data/ml100k/u.item', sep='|', header=None, names=col_names, engine='python')
